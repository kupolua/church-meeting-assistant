...
2. Перевірити Telegram-бота живим повідомленням (єдине, що не покрито ні тестами, ні
   перевіркою після cutover).
3. `migrate_tenant_fs.py` — коли захочеться однорідної розкладки (не обовʼязково: legacy-фолбек
