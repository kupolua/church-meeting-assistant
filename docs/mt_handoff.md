...
**Провалідовано:** `tests/mt_phase3_smoke.py` — **59 перевірок, усі PASS** на sandbox-БД
...
«не знайдено») і сесії (відкликання діє одразу, деактивація вибиває сесію, replay cookie після
...
1. `secure=True` на cookie + TLS, коли вийде за межі LAN (зараз `secure=False`).
2. Абсолютної стелі життя сесії немає — TTL відлічується від входу й не подовжується, але
   активна сесія живе всі 12 год. Якщо потрібно «не довше N годин попри все» — поле в
   `web_sessions` уже є місце.
3. Сторінка «мої сесії» для не-адміна (зараз лише адмін бачить чужі й свої).
4. **Live cutover (координовано, РУЙНІВНО):** створити роль `cma_app` + пароль → застосувати
   `003`+`004`+`005`+`006`+`007`+`008` до `cma` → у `.env`: `DB_USER=cma_app`, `DB_PASSWORD`,
...
> Прочитай `docs/mt_handoff.md`. Робимо [cutover / TLS / сторінка «мої сесії»].
