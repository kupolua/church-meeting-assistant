...
**`X-Forwarded-For` уже працює — і його НЕ треба вмикати.** Caddy додає заголовок
сам, а в uvicorn `--proxy-headers` увімкнений **за замовчуванням** (`config.py:219`)
з `forwarded_allow_ips = "127.0.0.1"` — рівно тією вузькою довірою, яка тут
потрібна. Тому `request.client.host` — це адреса відвідувача, а не проксі, і
гальмо входу (`routes/auth.py`, ключ `IP|логін`) рахує спроби по-справжньому
поклієнтно. Звірено на проді: у `web_sessions.ip` публічні адреси, а `127.0.0.1`
лишився тільки в рядках до 18.08, тобто з часів, коли веб працював на M1 без
проксі.

⚠️ **Ніколи не ставте `FORWARDED_ALLOW_IPS=*`.** Це виглядає як «дозволити
проксі», а насправді вмикає в uvicorn інший розбір: при `*` береться **перший**
запис `X-Forwarded-For` (`proxy_headers.py:176`), а не перший недовірений
справа. Caddy **додає** до заголовка, який надіслав клієнт, тож перший запис —
це те, що написав сам відвідувач. Ключ гальма стає підробним полем: ротуй
заголовок — і ліміт спроб пароля зникає. Поточна поведінка (справа наліво, до
першого недовіреного) саме тому й безпечна, і міняти її нема причин.

⚠️ **`restart_dev.sh` на VPS не запускати — з 01.09 він там відмовляється сам.**
Це помічник для ноутбука, який приїжджає сюди з кожним `git pull`. Він починає з
`pkill` по `uvicorn church_assistant.web.main:app` і `church_assistant.bot.main` —
тобто по живих сервісах церкви. `pkill` шле SIGTERM, uvicorn виходить **чисто**
(код 0), а в юніті `Restart=on-failure` — тож systemd його **не підніме**, і сайт
лишиться лежати. Далі скрипт запустив би їх повз systemd, а `uv run` без
`UV_NO_GROUP=worker` потягнув би torch і CUDA — 4.5 ГБ на машину без GPU.

Знайдено 01.09: `--check` там повідомив «Qdrant недоступний», поки Qdrant працював
— скрипт шукав контейнер з іменем із ноутбука. Хибна відповідь була **нешкідливою
половиною** того, що він тут робить. Тут потрібні `systemctl status/restart` і
`docker compose … ps`.
