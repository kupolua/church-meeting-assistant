...
### ⚠️ 4.5 ГБ CUDA у venv — єдина зайвина, за яку відповідаємо ми

`/srv/cma/.venv` містить `nvidia` (2.7 ГБ), `torch` (1.1 ГБ) і `triton` (698 МБ) на
машині **без відеокарти**. Ні веб, ні бот їх не імпортують — у `local_rerank.py`
`torch` вантажиться ліниво, всередині функції, а реранк живе на M1.

Видаляти руками **не можна**: `pyproject.toml` — один плаский список, і найближчий
`uv sync` поверне все назад. Тривке рішення — окремий extra для важкого
(`faster-whisper`, `pyannote-audio`, `sentence-transformers`, `omegaconf`), який на
VPS не ставиться.
