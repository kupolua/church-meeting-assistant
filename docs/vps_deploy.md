...
⚠️ **Не перезапускати `cma-web` «щоб перевірити»** — але не тому, що це виганяє
людей. Права перевіряються `sudo -n -u cma …`, бо рестарт нічого про них не
доводить, а не тому, що він дорогий.

**Виправлення (25.08): рестарт `cma-web` НЕ завершує сесії.** Тут раніше стояло
«кожен рестарт рве всі живі сесії», і це неправда — сесії з MT Phase 3 лежать
рядками в `web_sessions`, а не в пам'яті процесу. Перевірено на живій машині:
браузер із відкритим дашбордом опитував `/dashboard/panel` до рестарту о 16:20:40
і отримав `200` через дві секунди після нього, не логінячись. У пам'яті процесу
живе лише лічильник невдалих спроб (`_failures` в `auth.py`) — його рестарт
справді обнуляє, тобто **знімає блокування після 8 невдач**. Це єдине, що
рестарт «рве», і воно радше на користь заблокованому, ніж на шкоду.
