...
**Дата:** 24 серпня 2026 *(попередні редакції — 18 серпня, 29 липня; розділи нижче датовані)*
**Стан:** MVP-A (foundation + web + bot + worker), MVP-B (dashboard), MVP-C (ingestion), meeting-detail (стенограма + аудіоплеєр), UI-навігація, редагування спікерів **(включно з ручним додаванням тих, кого не почула діаризація)**, **повний self-host (bge-m3 + bge-reranker, Voyage прибрано)**, **мультитенантність у продакшні з 29.07** (див. `docs/mt_handoff.md`) і **PDF-експорт** (протоколи + власні Markdown-документи). Усе закомічено в `main` і запушено на GitHub.
...
**Ролі (`CMA_ROLE`, дефолт `all`).** Схема вище описує сьогоднішній стан: одна машина тримає все.
Після розʼїзду площин (`docs/vps_deploy.md`) на M1 треба виставити `CMA_ROLE=worker` — тоді
`restart_dev.sh` піднімає **лише два воркери**, не чіпає локальні контейнери (Postgres і Qdrant
переїжджають на VPS) і перевіряє їх доступність через тунель. web і Telegram-бот стають
systemd-юнітами на VPS. Поки `CMA_ROLE` не виставлено, поведінка не змінюється ані на йоту.
...
*(Оновлено 24.08.2026. Блокерів немає. Нумерація історична, не пріоритет: активний напрям зараз — **п.8** (розʼїзд площин), а **п.1** (калібрування) став його передумовою — без реранкера на VPS знадобиться другий набір порогів.)*
...
**1. Калібрування rerank-порогів + якість retrieval на bge** — S ← **найстаріший невиконаний борг**
   ...
   (у `.env` зараз 0.50/0.20 — підкручено на око, не за даними). Впливає лише на кольори в UI.
...
**8. Розʼїзд площин: VPS (control plane) + M1 (обробка)** — M, **код готовий, деплой попереду**
   Web + Telegram-бот + Postgres + Qdrant + артефакти на VPS; Ollama і обидва воркери — на M1.
   Мета — не швидкість, а **доступність**: церква вантажить о 23:00, не чекаючи, поки
   прокинеться ноутбук.
   - ✅ **Правка 1** (`1165ad0`): папка зустрічі шукається за (тенант, дата), не за збереженим шляхом.
   - ✅ **Правка 2** (`fc781a9`): `POST /api/query` ставить у чергу; веб більше **не виконує RAG**,
     отже йому не потрібні ні Ollama, ні реранкер, ні ваги Gemma.
   - ✅ **Ранбук і артефакти деплою** — `docs/vps_deploy.md` + `deploy/` (WireGuard, compose,
     systemd, скрипти міграції). `restart_dev.sh` отримав `CMA_ROLE=worker`; дефолт `all`
     лишає поведінку на M1 незмінною.
   - ⬜ **Виконати деплой** — кроки 1–10 ранбука. Виконує Pavlo вручну (доступу до VPS у Claude немає).
   - ⬜ **Правка 3 — синхронізація артефактів.** Для першого запуску ранбук пропонує NFS-маунт
     (нуль коду), але транскрипція тримає файл на 68 МБ через тунель **2–3 години**, і обрив
     вбиває найдорожчу фазу. Правильно — rsync на переходах стану (~60 рядків у `processor.py`,
     стан-машина вже серіалізує власність теки). Зробити до першої **чужої** церкви.
   - ⚠️ **VPS: 4× Zen 1 @2 ГГц, 8 ГБ, 68 ГіБ.** Web+Postgres+Qdrant ≈ 2.6 ГБ — влазить із запасом.
     Реранкер (torch + 568M fp32 ≈ 3.5 ГБ) **не влазить**. Whisper — десятки годин на зустріч.
     Це control plane назавжди, не сходинка до Spark.
   - ✅ **Аудіо зберігається** (рішення 24.08.2026): служителям потрібен доступ до запису після
     того, як протокол готовий. Наслідки — ~68 МБ на зустріч на диску VPS, і `cloud_plan` §2
     більше не обіцяє церкві, що найчутливіший артефакт зникає після обробки.
   - ⬜ **Бекап стає обовʼязком.** Після переїзду копія-джерело — VPS, а не ноутбук.
...
- Регресійний набір: `uv run python tests/mt_phase3_smoke.py` — **103 перевірки** (MT-ізоляція, auth, сесії, admin-UI, PDF-експорт, ручні спікери, переносність папки зустрічі, черга запитів, сканери джерел і шаблонів).
...
**Регресійний набір:** `uv run python tests/mt_phase3_smoke.py` — 103 перевірки, sandbox `cma_mt3`.
Живу базу `cma` він не чіпає. Останній зелений прогін — 24.08.2026.
