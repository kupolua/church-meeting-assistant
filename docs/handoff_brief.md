...
**1b. Реранк коштує 18 с там, де мав би 2** — S, **діагностовано, не полагоджено**
   ...
   **`bge-reranker-base` перевірено (24.08) — швидкість підтвердилась, якість недоведена:**

   | | top-1 | top-3 | після Gemma |
   |---|---|---|---|
   | `bge-reranker-v2-m3` (568M) | 9/20 | **15/20** | 17.72 с |
   | `bge-reranker-base` (278M) | 8/20 | 12/20 | **1.97 с** |

   Тест: заголовок теми як запит проти 20 кандидатів, правильний документ має стати першим.
   Менша модель влазить у те, що лишається після 17 ГБ Gemma, тому витіснення її майже
   не зачіпає — **9× у реальному сценарії**. Але 9 проти 8 у top-1 це шум на вибірці з 20,
   а 15 проти 12 у top-3 на такій вибірці недоказове. Система віддає Gemma **top-5**, тож
   саме ця частина й важить. **Не перемикати на цих даних** — потрібна ширша оцінка.

   ⚠️ І сам кандидат сумнівний: `bge-reranker-base` — **англо-китайська** модель, мультимовна
   в цій сімʼї лише `v2-m3`. Правильніший напрям — «розмір base, але мультимовна»:
   `jinaai/jina-reranker-v2-base-multilingual` (278M) або
   `Alibaba-NLP/gte-multilingual-reranker-base` (306M). Обидві міняються одним рядком
   `RERANK_MODEL` у `.env`, код чіпати не треба.

   Решта варіантів: менший квант Gemma, або прийняти 18 с як ціну співіснування на 32 ГБ
   (це ~20–30% запиту, решта — сама Gemma). Виносити реранк на VPS **не можна**:
