...
**1. Калібрування rerank-порогів + якість retrieval на bge** — S ← **найстаріший невиконаний борг**, і він підріс
   ⚠️ Після cutover'а видно, що реранк займає **21–29 с** проти задокументованих ~2 с на mps,
   тобто це вже друга за вартістю стадія після Gemma. У `.env` немає ні `RERANK_DEVICE`,
   ні `EMBED_MODEL` — перше, що варто перевірити: чи він узагалі не на CPU.
...
- Регресійний набір: `uv run python tests/mt_phase3_smoke.py` — **111 перевірок** (MT-ізоляція, auth, сесії, admin-UI, PDF-експорт, ручні спікери, переносність папки зустрічі, черга запитів, синхронізація артефактів, сканери джерел і шаблонів). Ганяється проти **локального** Postgres (`cma_mt3`), не проти VPS — прибито в шапці файла.
...
**Регресійний набір:** `uv run python tests/mt_phase3_smoke.py` — 111 перевірок, sandbox `cma_mt3`.
