...
**8. Розʼїзд площин: VPS (control plane) + M1 (обробка)** — M, **частково почато**
   Web + Postgres + Qdrant на власний VPS, ingestion- і query-воркери лишаються на M1.
   Мета — не швидкість, а **доступність**: церква вантажить о 23:00, не чекаючи, поки
   прокинеться ноутбук. Крос-tenant черга (`claim_next_ingestion_job`) і `DATA_ROOT` для цього
   вже готові; `OLLAMA_URL`/`QDRANT_URL`/`DB_HOST` — з `.env`.
   - ✅ **Правка 1 зроблена** (`1165ad0`): папка зустрічі шукається за (тенант, дата).
   - ⬜ **Правка 2:** `POST /query` виконує `rag.answer()` **синхронно в процесі веба** —
     на VPS це неможливо (Gemma 26B). Перевести на ту саму чергу `queries`, якою вже ходить
     Telegram. Зачіпає UX сторінки запиту, тож робити під конкретний переїзд.
   ...
   - ⬜ **Відкрите питання, і воно продуктове:** чи потрібен служителям доступ до запису після
     того, як протокол готовий? Після транскрипції аудіо читають **лише два плеєри** (сторінка
     зустрічі й редактор спікерів) — ні force-переобробка, ні ручні спікери, ні голосові профілі
     його не чіпають. Від відповіді залежать і обіцянка церкві в `cloud_plan` §2, і диск.
...
- Регресійний набір: `uv run python tests/mt_phase3_smoke.py` — **95 перевірок** (MT-ізоляція, auth, сесії, admin-UI, PDF-експорт, ручні спікери, переносність папки зустрічі, сканери джерел і шаблонів).
...
| розʼїзд web/обробка, VPS | backlog п.8 нижче + `cloud_plan.md` §13 |
...
**Регресійний набір:** `uv run python tests/mt_phase3_smoke.py` — 95 перевірок, sandbox `cma_mt3`.
