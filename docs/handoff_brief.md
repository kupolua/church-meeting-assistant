...
**8. Розʼїзд площин** — ✅ **ЗРОБЛЕНО 24.08**, у продакшні (`https://cma.rechurch.org.ua`).
   ...
   - ⬜ **Бекап.** Копія-джерело тепер VPS, а не ноутбук. Без бекапу це не покращення,
     а заміна одного сценарію втрати на інший. Мінімум: щоденний `pg_dump` + щотижневий
     rsync артефактів на інший майданчик.
...
- Регресійний набір: `uv run python tests/mt_phase3_smoke.py` — **112 перевірок** (MT-ізоляція, auth, сесії, admin-UI, PDF-експорт, ручні спікери, переносність папки зустрічі, черга запитів, синхронізація артефактів, сканери джерел і шаблонів). Ганяється проти **локального** Postgres (`cma_mt3`), не проти VPS — прибито в шапці файла.
...
**Регресійний набір:** `uv run python tests/mt_phase3_smoke.py` — 112 перевірок, sandbox `cma_mt3`.
